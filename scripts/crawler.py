...
class Crawler:
    ...
    def __init__(self, email: str, password: str) -> 'Crawler':
        ...
        # Adding the user-agent to make requests look like they are coming from a real browser
        ...
        # Headless mode: runs the browser without a GUI
        self.options.add_argument("--headless")
        self.options.add_argument("--disable-gpu")  # Disable GPU acceleration (often needed in headless mode)
        self.options.add_argument("--no-sandbox")  # Helps prevent some errors in headless mode, especially in Docker environments

        # Set the path for the ChromeDriver (managed by webdriver-manager)
    ...
    def crawl(self, companyName: str, departmentName: str, pageNum: int = 1) -> list:
        ...
        if(pageNum == 1):
            # Login to LinkedIn if on the first page
        ...
        # Random sleep to mimic human behavior and avoid being flagged
